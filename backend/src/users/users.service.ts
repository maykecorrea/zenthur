...
export class UsersService {
  ...
  async remove(id: number) {
    try {
      ...
      // ⭐ EXCLUSÃO EM CASCATA - SEM PERGUNTAS, APAGA TUDO!
      console.log('🗑️ Iniciando exclusão em cascata completa...');

      // 1. Excluir marcadores (via plantas do usuário)
      console.log('🗑️ Excluindo marcadores...');
      await this.prisma.marcador.deleteMany({
        where: {
          planta: {
            userId: id
          }
      ...
      console.log('✅ Marcadores excluídos');

      // 2. Excluir plantas
      console.log('🗑️ Excluindo plantas...');
      await this.prisma.planta.deleteMany({
        where: { userId: id }
      });
      console.log('✅ Plantas excluídas');

      // 3. Excluir documentos
      console.log('🗑️ Excluindo documentos...');
      await this.prisma.documento.deleteMany({
        where: { userId: id }
      });
      console.log('✅ Documentos excluídos');

      // 4. Excluir histórico de técnicos em manutenções
      console.log('🗑️ Excluindo histórico de técnicos...');
      await this.prisma.manutencaoTecnico.deleteMany({
        where: { tecnicoId: id }
      });
      console.log('✅ Histórico de técnicos excluído');

      // 5. Excluir manutenções
      console.log('🗑️ Excluindo manutenções...');
      await this.prisma.manutencao.deleteMany({
        where: { userId: id }
      });
      console.log('✅ Manutenções excluídas');

      // 6. Excluir equipamentos
      console.log('🗑️ Excluindo equipamentos...');
      await this.prisma.equipamento.deleteMany({
        where: { userId: id }
      });
      console.log('✅ Equipamentos excluídos');

      // 7. Excluir categorias
      console.log('🗑️ Excluindo categorias...');
      ...
      console.log(`✅ [Service] TUDO EXCLUÍDO! Usuário e todos os dados relacionados: ${user.email} (ID: ${id})`);
