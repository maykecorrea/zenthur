...
const handleLogin = async () => {
  ...
  try {
    ...
    if (response.success) {
      ...
      if (userRole === 'admin') {
        console.log("✅ Usuário é admin, salvando dados...");
        
        localStorage.setItem('admin_token', token);
        localStorage.setItem('admin_user', JSON.stringify(userData));
        
        console.log("✅ Dados salvos, redirecionando para /adm");
        await router.push('/adm');
        
      } else {
        console.warn("⚠️ Usuário não é admin. Role:", userRole);
        error.value = `Acesso negado. Apenas administradores podem acessar este painel. Seu papel: ${userRole}`;
      }
    } else {
