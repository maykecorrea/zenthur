...
const salvarEdicao = async (dadosEditados) => {
  try {
    ...
    const response = await fetch(`http://localhost:3001/api/manutencoes/${dadosEditados.id}`, {
      method: 'PUT',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json'
...
const arquivarManutencao = async () => {
  ...
  try {
    ...
    const response = await fetch(`http://localhost:3001/api/manutencoes/${props.maintenance.id}/arquivar`, {
      method: 'PATCH',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json'
